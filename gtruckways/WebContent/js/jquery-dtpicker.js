	...
	var draw = function($picker, option, year, month, day, hour, min) {
		...
		for (var zz = 0; i < cellNum; i++) {
			...
			if (shownDate.getFullYear() == pickedDate.getFullYear() && shownDate.getMonth() == pickedDate.getMonth() && realDay == pickedDate.getDate()) { /* selected day */
				$td.addClass('');//removed active class
			}
		...
		if ($picker.data("dateOnly") === true) {
			...
			while( hour_*100+min_ < maxTime[0]*100+maxTime[1] ){
				...
				var $o = $('<div>');
				var is_past_time = hour_ < (todayDate.getHours()+2) || (hour_ == (todayDate.getHours()+2) && min_ < (todayDate.getMinutes()));//to change time slot to one and half hour

			//	var is_past_time = hour_ < todayDate.getHours()+1 || (hour_ == todayDate.getHours()+1 && min_ < todayDate.getMinutes());
